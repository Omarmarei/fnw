...
def fund():
    if request.method == "POST":
        uidd =session["user_id"]
        cashcheck= cashcheck1()
        fund = request.form.get("cash")
        newcash = '%.2f'% (cashcheck + float(fund))
        db.execute("UPDATE users SET cash = ? WHERE id = ?", ( newcash, uidd ) )
        flash('Cash is added to your account successfully')
    ...
    if request.method == "POST":
        ...
        if stock == None:
            ...
            #buying stock
                ...
                uid = uidd, symbol = stock["symbol"], name = stock["name"], price = stock["price"], shares=amount, totalpayment = totalprice, operation="Bought", time = timex )
                db.execute("UPDATE users SET cash = ? WHERE id = ?", ( newcash, uidd ) )
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if int(len(newperson)) > 0:
        ...
        elif int(len(newperson)) == 0:
            z = request.form.get("password")
            c = True
            while c:
                if (len(z)<5 or len(z)>12):
                    break
            ...
            if c:
                flash("Not a Valid Password, password must be a combination of upper and lower case letters, numbers and symbols (@, #, $, %, etc.) ")
        ...
        if not sharecheck:
            ...
            for item in sharecheck2:
                ...
                if sharesto < float(amount):
                    ...
                    db.execute("INSERT INTO recordsop ( uid, symbol, name, price, shares, totalpayment, operation, time) VALUES (:uid, :symbol, :name, :price , :shares , :totalpayment, :operation, :time  )",
                    uid = uidd, symbol = stock["symbol"], name = stock["name"], price = stock["price"], shares= sharestosell, totalpayment = totalsale, operation="Sold", time = timex)
                    db.execute("UPDATE users SET cash = ? WHERE id = ?", ( newcash, uidd ) )
